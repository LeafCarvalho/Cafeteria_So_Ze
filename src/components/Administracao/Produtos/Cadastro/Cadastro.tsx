...
const Cadastro = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setFeedback(null);
    if (Number(formFields.valor) < 0 || !formFields.valor) {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        text: `“${formFields.nome}” foi cadastrado com sucesso.`,
      });
  ...
  return (
    <section className="cadastro-container" aria-labelledby="cadastro-title">
      <div className="cadastro-heading">
        <p className="eyebrow">Cardápio</p>
        <h2 id="cadastro-title">Cadastrar novo produto</h2>
        <p>Preencha os detalhes para incluir uma nova opção no cardápio.</p>
      </div>
      {feedback && (
        <p
      ...
      <Form onSubmit={handleSubmit} noValidate>
        <div className="form-grid">
          {Object.entries(formFields).map(([field, value]) => {
            ...
            const id = `produto-${typedField}`;
            const controlProps = {
              value,
              onChange: (
                e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
              ) => updateField(typedField, e.target.value),
              required: typedField !== "descricao",
            };
            return (
              <Form.Group
                className={
                  typedField === "descricao"
              ...
              >
                ...
                {typedField === "descricao" ? (
                  <Form.Control as="textarea" rows={4} {...controlProps} />
                ) : (
                  <Form.Control
                    type={
                      typedField === "valor"
                    ...
                    min={typedField === "valor" ? "0" : undefined}
                    step={typedField === "valor" ? "0.01" : undefined}
                    placeholder={
                      typedField === "imagem" ? "https://..." : undefined
                    }
                    aria-describedby={
                      typedField === "imagem" ? `${id}-help` : undefined
                    }
                    {...controlProps}
                  />
                )}
                {typedField === "imagem" && (
                  <Form.Text id={`${id}-help`}>
                    Use uma URL pública da imagem do produto.
                  </Form.Text>
                )}
              </Form.Group>
            );
        ...
        <Button className="submit-product" type="submit" disabled={salvando}>
