...
const Inicio: React.FC = () => {
  ...
  const options: ChartOptions<"bar"> = {
    ...
    scales: {
      ...
      y: {
        beginAtZero: true,
        ticks: { color: "#f6f1e8", precision: 0 },
        grid: { color: "#31403b" },
      },
    },
    plugins: {
      ...
      {!loading && !erro && (
        <>
          <div
            ...
            aria-label="Totais de pedidos por período"
          >
            {Object.entries(pedidosCount).map(([periodo, quantidade]) => (
              <Card key={periodo} className="summary-card">
                <Card.Body>
                  ...
                  <Card.Text>
                    {quantidade}{" "}
                    <span>{quantidade === 1 ? "pedido" : "pedidos"}</span>
                  </Card.Text>
                </Card.Body>
              </Card>
            ))}
