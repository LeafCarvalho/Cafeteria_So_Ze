...
const Efetuacao = () => {
  ...
  const [orderConfirmation, setOrderConfirmation] =
    useState<UltimoPedido | null>(
      lastOrderConfirmation?.confirmacao_id === confirmacaoId
        ? lastOrderConfirmation
        : null,
    );
  ...
  const recoverOrder = useCallback(
    async (pickupCodeToValidate: string) => {
      ...
      try {
        const confirmation = await pedidosService.recuperarConfirmacao(
          confirmacaoId,
          pickupCodeToValidate,
        );
        if (!confirmation) {
          setOrderConfirmation(null);
          setErrorMessage(
            "Não encontramos um pedido com esses dados. Confira o código e tente novamente.",
          );
          return;
        }
  ...
  useEffect(() => {
    if (
      confirmacaoId &&
      confirmationAccessData?.confirmacaoId === confirmacaoId &&
      !orderConfirmation
    ) {
  ...
  if (!confirmacaoId) {
    return (
      <main className="efetuacao-container">
        <Container>
          <section
            className="efetuacao-card efetuacao-card--center"
            role="alert"
          >
      ...
      <main className="efetuacao-container">
        <Container>
          <section
            ...
            className="efetuacao-card efetuacao-card--recovery"
          >
            ...
            <Form noValidate onSubmit={handleSubmit}>
              <Form.Group controlId="codigoRetirada">
                ...
                <Form.Control
                  ...
                  value={pickupCode}
                />
                <p className="field-hint" id="ajuda-codigo">
                  São os 6 números enviados ao concluir o pedido.
                </p>
              </Form.Group>
              {errorMessage && (
                <div
                  className="efetuacao-feedback"
                  ref={feedbackRef}
                  role="alert"
                  tabIndex={-1}
                >
  ...
  return (
    <main className="efetuacao-container">
      <Container>
        <section aria-labelledby="confirmacao-title" className="efetuacao-card">
          <header className="efetuacao-card__header">
            <p className="efetuacao-eyebrow">Pedido confirmado</p>
            <h1 id="confirmacao-title">
              Tudo certo, {orderConfirmation.nome_cliente}.
            </h1>
            <p>Estimativa de preparo: 30 a 60 minutos.</p>
          </header>
          <div
            className="codigo-retirada"
            aria-label={`Código de retirada: ${orderConfirmation.senha_retirar_ped}`}
          >
            <span>Seu código de retirada</span>
            <strong>{orderConfirmation.senha_retirar_ped}</strong>
            <small>
              Válido até{" "}
              {new Date(orderConfirmation.expira_em).toLocaleTimeString(
                "pt-BR",
          ...
          <div className="confirmacao-itens">
            <h2>Seu pedido</h2>
            {orderConfirmation.produtos.map((item) => (
              <article key={item.id} className="pedido-item">
                <img src={item.imagem} alt="" />
                <div>
                  <h3>{item.nome}</h3>
                  <p>Quantidade: {item.quantidade}</p>
                </div>
                <strong>{formatCurrency(item.valor * item.quantidade)}</strong>
              </article>
            ))}
          </div>
          ...
          <DefaultButton
            customizarCSS="efetuacao-return"
            onClick={() => navigate("/")}
            type="button"
          >
            Voltar ao cardápio
